...
@Component
class WSRandomChatHandler(private val randomChatManager: RandomChatManager): TextWebSocketHandler() {
    private val logger = LoggerFactory.getLogger(javaClass)

    override fun afterConnectionEstablished(session: WebSocketSession) {
        runCatching {
            val token = session.authorizationToken?: throw RuntimeException("인증 토큰 없음")
            ...
            logger.error("소켓 연결 후 처리 오류 발생", e)
