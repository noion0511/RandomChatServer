...
import an.example.randomchat.controller.request.MessageRequset
...
class MessageController(
    ...
    @PostMapping("/message")
    fun sendMessage(@RequestBody requset: MessageRequset): ApiResponse {
        return try {
            val userId = userContextHolder.id
            val user = userRepository.findById(userId)
                    ?: throw RandomChatException("사용자 정보 없음.")

            randomChatManager.sendMessage(user, requset.content)
