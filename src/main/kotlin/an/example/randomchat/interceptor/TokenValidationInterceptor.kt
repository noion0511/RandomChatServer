...
//매번 요청할 때, 토큰을 검증하고 스레드로컬 변수에 사용자 정보를 저장해주는 기능
...
class TokenValidationInterceptor(private val userContextHolder: UserContextHolder) : HandlerInterceptor {
    ...
    override fun preHandle(request: HttpServletRequest, response: HttpServletResponse, handler: Any): Boolean {
        ...
        if(authHeader.isNullOrBlank()) {
            ...
            return handlerToken(grantType, token, response)
        }
    }

    private fun extractToken(token: String) = token.replace(BEARER, "").trim()

    private fun handlerToken(grantType: String?, token: String, response: HttpServletResponse) =
            try {
